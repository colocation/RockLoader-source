(function(){
  ...
  Chart.HorizontalRectangle = Chart.Element.extend({
		draw : function(){
			...
			ctx.moveTo(this.left + 10, top);
  			ctx.lineTo(right + 10, top);
  			ctx.lineTo(right + 10, bottom);
			ctx.lineTo(this.left + 10, bottom);
			ctx.fill();
			if (this.showStroke){
				ctx.stroke();
			}
	...
	Chart.Type.extend({
        ...
        calculateBaseHeight : function(){
				...
				calculateXInvertXY : function(value) {
					var scalingFactor = (this.width -10 - Math.round(this.xScalePaddingLeft) - this.xScalePaddingRight) / (this.max - this.min);
        ...
        calculateX : function(index){
    			var isRotated = (this.xLabelRotation > 0),
    				innerWidth = this.width - 10 - (this.xScalePaddingLeft + this.xScalePaddingRight),
    			...
    			return Math.round(valueOffset) + 10;
    		},

        draw : function(){
    			var ctx = this.ctx,
    				yLabelGap = (this.endPoint - this.startPoint) / this.yLabels.length,
    				xStart = Math.round(this.xScalePaddingLeft) + 10;
    			if (this.display){
    				...
    				helpers.each(this.yLabels,function(labelString,index){
                        ...
                        if(drawHorizontalLine){
                            ...
                            ctx.lineTo(this.width-5, linePositionY);
                            ctx.stroke();
                            ctx.closePath();
                        }
